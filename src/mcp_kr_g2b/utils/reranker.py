...
- sentence-transformers 는 함수 내부에서 지연 임포트하므로, 미설치 시에도 서버 import 및
  나머지 도구는 정상 동작한다.
- 모델: 기본 한국어 문장 임베딩(jhgan/ko-sroberta-multitask). G2B_RERANK_MODEL 로 변경.
- 스레드 안전(이중 검사 락)이며, 모델명 변경 시 재로딩한다.
"""
...
def _max_candidates() -> int:
    try:
        return max(1, int(os.getenv("G2B_RERANK_MAX_CANDIDATES", "2000")))
...
def rerank(
    ...
    if not items or top_k is None or top_k <= 0:
        return []

    cap = _max_candidates()
    capped = items[:cap]
    if len(items) > cap:
        logger.warning(f"리랭크 후보 {len(items)}건 → 상한 {cap}건만 평가(G2B_RERANK_MAX_CANDIDATES)")

    from sentence_transformers import util

    model = _get_model()
    texts = [str(it.get(text_field, "") or "") for it in capped]
    ...
    scored = sorted(zip(capped, sims), key=lambda x: x[1], reverse=True)
