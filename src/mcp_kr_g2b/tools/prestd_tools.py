"""나라장터 사전규격정보서비스 도구 (코드 생성)

서비스 ID : HrcspSsstndrdInfoService
오퍼레이션 : 20개
호출       : get_prestd_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("prestd", "<operation>")
"""
...
@mcp.tool(
    name="get_prestd_data",
    description=build_service_tool_description("prestd"),
    tags={"조달청", "나라장터", "사전규격"},
)
def get_prestd_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getPublicPrcureThngInfoThng (사전규격 물품 목록 조회)")],
    params: Annotated[
    ...
    """나라장터 사전규격정보서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("prestd", operation, params, fetch_all, num_of_rows, page_no)
