"""나라장터 계약정보서비스 도구 (코드 생성)

서비스 ID : CntrctInfoService
오퍼레이션 : 21개
호출       : get_contract_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("contract", "<operation>")
"""
...
@mcp.tool(
    ...
    description=build_service_tool_description("contract"),
    tags={"조달청", "나라장터", "계약정보"},
)
def get_contract_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getCntrctInfoListThng (계약현황에 대한 물품조회)")],
    params: Annotated[
    ...
    """나라장터 계약정보서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("contract", operation, params, fetch_all, num_of_rows, page_no)
