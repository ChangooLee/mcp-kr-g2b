"""나라장터 사용자정보서비스 도구 (코드 생성)

서비스 ID : UsrInfoService02
오퍼레이션 : 5개
호출       : get_user_info_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("user_info", "<operation>")
"""
...
@mcp.tool(
    name="get_user_info_data",
    description=build_service_tool_description("user_info"),
    tags={"조달청", "나라장터", "사용자정보"},
)
def get_user_info_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getDminsttInfo02 (수요기관정보조회)")],
    params: Annotated[
    ...
    """나라장터 사용자정보서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("user_info", operation, params, fetch_all, num_of_rows, page_no)
