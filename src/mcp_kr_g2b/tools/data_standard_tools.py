"""나라장터 공공데이터개방표준서비스 도구 (코드 생성)

서비스 ID : PubDataOpnStdService
오퍼레이션 : 3개
호출       : get_data_standard_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("data_standard", "<operation>")
"""
...
@mcp.tool(
    name="get_data_standard_data",
    description=build_service_tool_description("data_standard"),
    tags={"조달청", "나라장터", "개방표준"},
)
def get_data_standard_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getDataSetOpnStdBidPblancInfo (데이터셋 개방표준에 따른 입찰공고정보)")],
    params: Annotated[
    ...
    """나라장터 공공데이터개방표준서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("data_standard", operation, params, fetch_all, num_of_rows, page_no)
