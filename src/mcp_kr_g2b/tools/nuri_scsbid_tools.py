"""누리장터 민간낙찰정보서비스 도구 (코드 생성)

서비스 ID : PrvtScsbidInfoService
오퍼레이션 : 7개
호출       : get_nuri_scsbid_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("nuri_scsbid", "<operation>")
"""
...
@mcp.tool(
    name="get_nuri_scsbid_data",
    description=build_service_tool_description("nuri_scsbid"),
    tags={"조달청", "누리장터", "민간낙찰"},
)
def get_nuri_scsbid_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getPrvtScsbidListSttus (민간 낙찰된 목록 현황 조회)")],
    params: Annotated[
    ...
    """누리장터 민간낙찰정보서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("nuri_scsbid", operation, params, fetch_all, num_of_rows, page_no)
