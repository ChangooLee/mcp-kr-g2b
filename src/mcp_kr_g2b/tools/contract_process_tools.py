"""나라장터 계약과정통합공개서비스 도구 (코드 생성)

서비스 ID : CntrctProcssIntgOpenService
오퍼레이션 : 4개
호출       : get_contract_process_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("contract_process", "<operation>")
"""
...
@mcp.tool(
    name="get_contract_process_data",
    description=build_service_tool_description("contract_process"),
    tags={"조달청", "나라장터", "계약과정공개"},
)
def get_contract_process_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getCntrctProcssIntgOpenFrgcpt (계약과정통합공개정보에 대한 외자조회)")],
    params: Annotated[
    ...
    """나라장터 계약과정통합공개서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("contract_process", operation, params, fetch_all, num_of_rows, page_no)
