"""나라장터 낙찰정보서비스 도구 (코드 생성)

서비스 ID : ScsbidInfoService
오퍼레이션 : 23개
호출       : get_scsbid_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("scsbid", "<operation>")
"""
...
@mcp.tool(
    name="get_scsbid_data",
    description=build_service_tool_description("scsbid"),
    tags={"조달청", "나라장터", "낙찰정보"},
)
def get_scsbid_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getScsbidListSttusThng (낙찰된 목록 현황 물품조회)")],
    params: Annotated[
    ...
    """나라장터 낙찰정보서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("scsbid", operation, params, fetch_all, num_of_rows, page_no)
