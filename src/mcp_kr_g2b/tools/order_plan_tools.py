"""나라장터 발주계획현황서비스 도구 (코드 생성)

서비스 ID : OrderPlanSttusService
오퍼레이션 : 8개
호출       : get_order_plan_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("order_plan", "<operation>")
"""
...
@mcp.tool(
    name="get_order_plan_data",
    description=build_service_tool_description("order_plan"),
    tags={"조달청", "나라장터", "발주계획"},
)
def get_order_plan_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getOrderPlanSttusListThng (발주계획현황에 대한 물품조회)")],
    params: Annotated[
    ...
    """나라장터 발주계획현황서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("order_plan", operation, params, fetch_all, num_of_rows, page_no)
