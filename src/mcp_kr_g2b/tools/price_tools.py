"""나라장터 가격정보현황서비스 도구 (코드 생성)

서비스 ID : PriceInfoService
오퍼레이션 : 11개
호출       : get_price_data(operation="<영문오퍼레이션명>", params={...})
상세 스펙  : get_g2b_operation_info("price", "<operation>")
"""
...
@mcp.tool(
    name="get_price_data",
    description=build_service_tool_description("price"),
    tags={"조달청", "나라장터", "가격정보"},
)
def get_price_data(
    operation: Annotated[str, Field(description="오퍼레이션명(영문). 예: getPriceInfoListFcltyCmmnMtrilEngrk (시설공통자재(토목) 가격정보)")],
    params: Annotated[
    ...
    """나라장터 가격정보현황서비스 의 오퍼레이션을 호출하여 결과를 캐시하고 요약을 반환합니다."""
    return dispatch_service_fetch("price", operation, params, fetch_all, num_of_rows, page_no)
